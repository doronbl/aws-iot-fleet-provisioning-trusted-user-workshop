...
IoT Atlast [DEVICE BOOTSTRAP - TRUSTED USER](https://iotatlas.net/en/implementations/aws/device_bootstrap/fleet_provisioning_trusted_user/) - Describe the device side of the flow. Please note that the CreateProvisioningClaim to AWS IoT Core is a control plane API call and not data plane API call, similar to any other AWS service API call. It is adviced to decouple the mobile app from the AWS IoT data plane APIs such as: command & control, and telemetry. These in turn should be proxied by API Gateway and Lambda calls. Decoupling mobile app from the data plane have many benefits, such as:
